...
function buildTourQuotationHtml(data, options = {}) {
  ...
  const logoSrc = options.logoDataUri || getLogoDataUri() || '';
  const imgFallback = logoSrc || TRANSPARENT_PIXEL;

  const heroMain = d.heroMain && String(d.heroMain).startsWith('data:') ? d.heroMain : imgFallback;
  const heroSub1 = d.heroSub1 && String(d.heroSub1).startsWith('data:') ? d.heroSub1 : imgFallback;
  const heroSub2 = d.heroSub2 && String(d.heroSub2).startsWith('data:') ? d.heroSub2 : imgFallback;
